...
--- Generate title string for window title
--- Shows: hostname (on linux) + file path + last modified time
--- @return string
function M.get_titlestr()
  ...
  -- Add last modified time for listed buffers with a path
  if vim.bo.buflisted and buf_path ~= "" then
    local ftime = vim.fn.getftime(vim.fn.expand("%"))
    if ftime > 0 then
      title = title .. os.date("%Y-%m-%d %H:%M:%S", ftime)
    end
  end
